<?php
    ...
    class DAOorcamento extends Orcamento{
		...
		public function lista_orc(){
			...
			try {
				...
				foreach ($res as $l) {
					...
					echo $this->getFone_cli_orc();
					echo '"></td><td class="tdinput"><input type="submit" name="sobe_ver_orc" value="Listar" class="altera"/><input type="submit" name="exclui_orc" value="Excluir" class="exclui"/></form></td></tr>';
		...
		public function exclui_orc(){
			...
			$this->setId_orc($id);
			$iten = isset($_POST['id_prod']) ? $_POST['id_prod'] : null;
			
			try{
			
				if(!empty($iten)){
				
					$sql_1 = 'DELETE FROM tborcamento_i WHERE cod_orc = :id_orc AND id_prod = "'.$iten.'"';
